...
<div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:24px;">
    <h2 style="font-family:'Bebas Neue', sans-serif; font-size:32px; letter-spacing:3px;">Editar Inventario</h2>
...
<div style="background:#fff; border:1px solid #eee; padding:32px; max-width:600px;">
    <form action="{{ route('inventarios.update', $inventario) }}" method="POST">
        ...
        <div class="sl-form-group">
            ...
            <input type="number" name="stock_actual" class="sl-input" value="{{ old('stock_actual', $inventario->stock_actual) }}" min="0" required>
        </div>
        <div class="sl-form-group">
            <label class="sl-label">Stock Mínimo</label>
            <input type="number" name="stock_minimo" class="sl-input" value="{{ old('stock_minimo', $inventario->stock_minimo) }}" min="0" required>
        </div>
        <div class="sl-form-group">
            <label class="sl-label">Stock Máximo</label>
            <input type="number" name="stock_maximo" class="sl-input" value="{{ old('stock_maximo', $inventario->stock_maximo) }}" min="0" required>
        </div>
